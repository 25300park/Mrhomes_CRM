...
test('admin responses, browser logs, and outbound requests never expose private reflection text', async ({ page }) => {
  ...
  expect(state.calls.some(call => call.operation === 'insert' && call.target === 'time_jobs')).toBe(true)
  expect(JSON.stringify(state.calls)).not.toContain(privateText)
  expect(JSON.stringify(state.logs)).not.toContain(privateText)
  expect(JSON.stringify(state.outbound)).not.toContain(privateText)
