...
describe('time-management database contract', () => {
  ...
  test('defines every approved time-management table', () => {
    ...
    expect(schema).toMatch(/linked_entity_id\s+is\s+not\s+distinct\s+from\s+coalesce\s*\(\s*contact_id\s*,\s*listing_id\s*,\s*lead_id\s*,\s*deal_id\s*\)/i)
    ...
    expect(schema).toMatch(/lease_token\s+uuid/i)
    expect(schema).toMatch(/max_attempts\s+integer\s+not\s+null\s+default\s+4/i)
    expect(schema).toMatch(/business_time_zone\s*=\s*'Asia\/Seoul'/i)
    expect(schema).not.toMatch(/uuid_generate_v4/i)
    expect(schema).toMatch(/create\s+extension\s+if\s+not\s+exists\s+btree_gist/i)
    expect(schema).not.toMatch(/create\s+extension[^;]*uuid-ossp/i)
  ...
  test('defines hardened atomic timer and queue functions', () => {
    ...
    expect(functions).toMatch(/lease_until\s*>\s*pg_catalog\.now\s*\(\s*\)/i)
    expect(functions).toMatch(/lease_token\s*=\s*pg_catalog\.gen_random_uuid\s*\(\s*\)/i)
    expect(functions).toMatch(/and\s+lease_token\s*=\s*p_lease_token/i)
    expect(functions).toMatch(/time_fail_job\s*\(\s*p_job_id\s+uuid\s*,\s*p_worker_id\s+text\s*,\s*p_lease_token\s+uuid/i)
    expect(functions).toMatch(/time_complete_job\s*\(\s*p_job_id\s+uuid\s*,\s*p_worker_id\s+text\s*,\s*p_lease_token\s+uuid/i)
