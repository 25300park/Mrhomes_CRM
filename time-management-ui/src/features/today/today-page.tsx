...
export function TodayPage({ api = apiClient, online = window.navigator.onLine, requestId = commandId, now = currentTime }: TodayPageProps) {
    ...
    async function load() {
      try {
        const [categoryData, planData] = await Promise.all([
          api.get('/categories'),
          api.get('/plans/today')
        ]) as [{ standard: Category[] }, { plan: { available_minutes: number } | null, varianceMinutes: number }]
        if (cancelled) return
        setCategories(categoryData.standard)
        setSelectedCategoryId(current => current || categoryData.standard[0]?.id || '')
        if (planData.plan) setAvailableMinutes(String(planData.plan.available_minutes))
        setVariance(planData.varianceMinutes || null)
        if (online) await reconcile()
      } catch {
  ...
  async function reconcile() {
    ...
    if (response.authoritativeEntry) {
      ...
      if (!response.matches) setMessage('Timer display updated from the server.')
    } else if (current) {
      setTimer(current)
  ...
  async function savePlan() {
    if (!online) return
    if (!selectedCategoryId) return setError('Choose a category before saving the plan.')
    setError('')
    try {
      const result = await api.put('/plans/today', {
        availableMinutes: Number(availableMinutes),
        allocations: [{ standardCategoryId: selectedCategoryId, plannedMinutes: Number(plannedMinutes) }]
  ...
  async function timerCommand() {
    if (!online) return setMessage('Offline: timer commands are unavailable; the running display remains visible.')
  ...
  return <section className="workflow-page" aria-labelledby="today-heading">
    ...
    {!online && <p className="offline-notice" role="status">Offline: plan, manual entry, and reflection changes are unavailable.</p>}
    ...
    <section className="workflow-card" aria-labelledby="timer-heading">
      ...
      {selectedCrm && <p>Linked: {selectedCrm.type}: {selectedCrm.label}</p>}
      {crmResults.length > 0 && <div aria-label="CRM search results">{crmResults.map(result => <button key={`${result.type}-${result.id}`} role="option" onClick={() => { setSelectedCrm(result); setCrmQuery(''); setCrmResults([]) }}>{result.type}: {result.label}</button>)}</div>}
      <div className="timer-actions">
        <button className="primary-timer" onClick={() => void timerCommand()}>{timer ? 'Switch timer' : 'Start timer'}</button>
        {timer && <button className="stop-timer" onClick={() => void stopTimer()}>Stop timer</button>}
      </div>
    </section>
    ...
    <section className="workflow-card" aria-labelledby="plan-heading">
      ...
      <label>Planned minutes<input aria-label="Planned minutes" type="number" min="0" max="1440" value={plannedMinutes} onChange={event => setPlannedMinutes(event.target.value)} /></label>
